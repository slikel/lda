...
public class LDADataset {
	 ...
	 */
	public static LDADataset readDataSet(List<String> list){
		try {
			int M = list.size();
			...
			for (int i = 0; i < list.size(); ++i){
				...
				Lexeme l = null;
				while((l=segmenter.next())!=null) {
					sb.append(l.getLexemeText());
					sb.append(" ");
				}
				data.setDoc(sb.toString().trim(), i);
			}
			return data;
		}
		catch (Exception e){
	 ...
	 *  @return dataset if success and null otherwise
	 */
	public static LDADataset readDataSet2(BufferedReader reader){
		try {
